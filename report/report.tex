...
\begin{algorithm}[H] 
	...
	\begin{algorithmic} 
		\STATE Data: $\{v_0, \hdots, v_{n-1}\}$ set of 2-D points
        \WHILE{Edges included != (numberof(vertices) - 1)} 
            \STATE Apply Union Find to Determine if smallest edge remaining makes a Cycle
            \IF{least expensive edge does not create a cycle based on union find}
                \STATE Add Edge to Minimum Spanning Tree
            \ENDIF
        \ENDWHILE
        \WHILE{Unvisited Nodes in set remain}
            \STATE Determine the Deepest Unexplored Vertex in the MST as assign it to $n$
            \FORALL{Outbound edges from node $n$ }
                \IF{Node Connected to the Outgoing Edge has not been visited}
                    \STATE Set the Node to visited
                    \IF{Next edge from node $n$ can be explored}
